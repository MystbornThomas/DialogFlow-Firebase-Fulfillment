...
function processV1Request (request, response) {
    ...
    let userInputString = request.body.result.resolvedQuery; // The text they typed in
    let timestamp = request.body.timestamp; // The typestamp the request was received
    const googleAssistantRequest = 'google'; // Constant to identify Google Assistant requests
    const app = new DialogflowApp({request: request, response: response});

    // Create handlers for Dialogflow actions as well as a 'default' handler
    // TODO: Turn this into a function that returns the proper class, instead of instantiating all of them
    ...
    actionHandlerInstance.setData(action, parameters, inputContexts, requestSource, userInputString, timestamp);

    if (requestSource === googleAssistantRequest) {
        sendGoogleResponse(actionHandlerInstance.getGoogleResponse()); // Send response to user
    } else {
        sendResponse(actionHandlerInstance.getNormalResponse()); // Send response to user
    }
